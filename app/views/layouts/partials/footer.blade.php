<footer class="footer uk-scrollspy-inview"
        data-uk-scrollspy="{cls:'uk-animation-fade', repeat: true}">
    <div class="row uk-scrollspy-inview"
         data-uk-scrollspy="{cls:'uk-animation-slide-bottom', repeat: true}">

        <!--        <p class="center">{{ App::getLocale() == 'en' ? HTML::image("logo_lg_en.png") : HTML::image("logo_lg_mn.png")}}</p>-->

        <div class="col-md-3  col-xs-offset-1 ">
            <h4><span class="glyphicon glyphicon-map-marker"></span> Ulaanbaatar, Mongolia.</h4>

            <p> Bayangold district </p>

            <p> Narni khoroolol </p>

            <p> Building #6 </p>
        </div>

        <div class="col-md-3 col-xs-offset-1">
            <h4><span class="glyphicon glyphicon-earphone"></span> Want to talk?</h4>

            <p> mobile: +976-8880-7711</p>

            <p> mobile: +976-9482-1983 </p>

            <p> mobile: +976-9907-7729 </p>
        </div>

        <div class="col-md-3 col-xs-offset-1 quick-access">
            <h4><span class="glyphicon glyphicon-envelope"></span> Stay connected.</h4>
            <p> email: <a href="mailto:Hello@greatgoldstar.com">Hello@greatgoldstar.com</a></p>
            <br>
            <p><a href="https://www.facebook.com/greatgoldstar"><i class="fa fa-facebook"></i></a>
                <a href="https://twitter.com/GreatGoldStar"><i class="fa fa-twitter"></i></a>
                <a href="https://www.linkedin.com/company/4799377?trk=vsrp_companies_cluster_name&trkInfo=VSRPsearchId%3A3555133281414502375670%2CVSRPtargetId%3A4799377%2CVSRPcmpt%3Acompanies_cluster"><i class="fa fa-linkedin"></i></a>
            </p>
        </div>

    </div>
    <p class="center" style="font-size: 0.8em; margin: 30px;">GGSC | Copyright ©2014 | All Rights Reserved
    </p>
</footer>